...
function getStitchData(height, width, stImg, stitchPatterns) {
	let data;
	...
	const processImage = new Promise((resolve) => {
		...
		if (!stImg) { //only one stitch pattern
			...
			console.log('!!!');
			let colors = [[255, 255, 255]]; //background

			for (let i = 0; i < stitchPatterns.length; ++i) { //new //*
				colors.push(stitchPatterns[i].color); //check
			}
