...
if (shape_code_reverse !== null) {
	...
	if (shaping_arr.length === 1) { //*//*//*
		shaping_arr.push(
			...
			SHAPING({
				ROW: first_short_row - 1, //TODO: change these to -1 and just first_short_row //?
				LEFT: 0,
				RIGHT: 0,
			})
		);
...
for (let i = 0; i < in_file.length; ++i) {
	...
	pass_loop: for (let p = 0; p < in_file[i].length; ++p) {
		...
		// if (shapeifyIgnore) { //new //*
		if (shapeifyIgnore || type.includes('x-') || (type === 'rack')) { //new //*
		...
		if (type !== 'xfer' && type !== 'drop' && !extension) { //* TODO: see how xfer and drop are handled
			...
			dir = type; //new //check //* //go back! //? (since separates)
		}
...
function findCarrierSide(c, row, passIdx) {
	...
	// let recentRow = Math.max.apply(null, carrierRows.filter(function(r){return r <= row}));
	// console.log(recentRow); //remove //debug
	// if (passIdx && recentRow == row) {

	// }
	// else return carrierRows[recentRow].side;
	// if (carrierRows.includes(row)) {

	// } else {
	// 	return carrier_track[carrierRows.find(r => Number(r) < )]
	// }
}

function findCarrierOnSide(side, row, passIdx, returnAll) {
	// let cs = Object.keys(carrier_track);
	let sideCs = [];
...
const dec1DoubleBed = (dec_needle, side) => { ////if double bed, need to just do it twice //TODO: double check with emptyRow
	...
	if (side === 'left') {
		...
		// } else notDec.push(`b${dec_needle}`); //*
		// // xfer_section.push('rack 1');
		// if (!emptyRow.includes(`b${dec_needle}`)) { //new //check //*
		// // if (!emptyRow.includes(`b${dec_needle}`) && !emptyRow.includes(`f${dec_needle + 1}`)) { //new //check //*
		// 	if (!emptyRow.includes(`f${dec_needle + 1}`)) {
		// 		xfer_section.push('rack 1');
		// 		xfer_section.push('x-add-roller-advance 150');
		// 		xfer_section.push(`xfer b${dec_needle} f${dec_needle + 1}`);
		...
		// 		// 	if (!emptyRow.includes(`f${dec_needle + 2}`)) xfer_section.push(`xfer b${dec_needle + 1} f${dec_needle + 2}`); //come back!
		// 		// 	else notDec.push(`b${dec_needle + 1}`); //new //*
		// 		// }
		// 	} else {
		...
		// 			xfer_section.push('rack 0');
		// 			xfer_section.push(`xfer f${dec_needle + 1} b${dec_needle + 1}`);
		// 		} else notDec.push(`b${dec_needle}`); //* //TODO: dec this with diff method if possible
		// 	}
		// }

		if (!emptyRow.includes(`b${dec_needle + 1}`) && !emptyRow.includes(`f${dec_needle + 2}`)) { //new //check //*
			if (racking !== 1) {
				racking = 1;
				xfer_section.push('rack 1'); //put rack back to 1
			}
			// if (emptyRow.includes(`f${dec_needle + 1}`)) xfer_section.push('rack 1'); //put rack back to 1
			xfer_section.push(`xfer b${dec_needle + 1} f${dec_needle + 2}`); //come back!
		}

		if (!emptyRow.includes(`f${dec_needle}`) && (!emptyRow.includes(`b${dec_needle + 1}`) || !emptyRow.includes(`f${dec_needle + 1}`))) { //new //check //*
		// if (!emptyRow.includes(`b${dec_needle + 1}`) || !emptyRow.includes(`f${dec_needle + 1}`)) {
			racking = -1;
			xfer_section.push('rack -1');
		...
		// 		}
		// 		notDec.push(`f${dec_needle}`);
		// 	}
		// }
	}
	if (side === 'right') {
		if (!emptyRow.includes(`b${dec_needle}`) && (!emptyRow.includes(`f${dec_needle - 1}`) || !emptyRow.includes(`b${dec_needle - 1}`))) { //new //check //*
			racking = -1;
			xfer_section.push('rack -1');
			xfer_section.push(`x-add-roller-advance 150`);
			xfer_section.push(`xfer b${dec_needle} f${dec_needle - 1}`);
...
const dec2DoubleBed = (dec_needle, side) => { //TODO: check this
	let racking = 0;
	if (emptyRow.length) xfer_section.push('x-carrier-stopping-distance 3.5'); //new //check //*
	...
	if (side === 'left') {
		...
		if (!emptyRow.includes(`b${dec_needle + 3}`) && !emptyRow.includes(`f${dec_needle + 3}`)) { //new //check //* //no
			xfer_section.push(`xfer b${dec_needle + 3} f${dec_needle + 3}`); //* f${dec_needle + 3} //*
		}

		if (!emptyRow.includes(`f${dec_needle}`) && !emptyRow.includes(`b${dec_needle + 1}`)) { //new //check //* //no
			racking = -1;
			xfer_section.push('rack -1');
			xfer_section.push('x-add-roller-advance 150');
		...
		if (!emptyRow.includes(`f${dec_needle + 1}`) && !emptyRow.includes(`b${dec_needle + 2}`)) { //new //check //*
			if (racking !== -1) {
				racking = -1;
				xfer_section.push('rack -1');
			}
		...
		if (!emptyRow.includes(`b${dec_needle + 3}`) && (!emptyRow.includes(`f${dec_needle + 2}`) || !emptyRow.includes(`b${dec_needle + 2}`))) { //new //check //*
			if (racking !== -1) {
				racking = -1;
				xfer_section.push('rack -1');
			}
			xfer_section.push(`xfer f${dec_needle + 2} b${dec_needle + 3}`); //* b${dec_needle + 3} //* //good!
		}
		...
		if (!emptyRow.includes(`b${dec_needle + 1}`) && (!emptyRow.includes(`f${dec_needle + 2}`) || !emptyRow.includes(`b${dec_needle + 2}`))) { //new //check //?
			if (racking !== 1) {
				racking = 1;
				xfer_section.push('rack 1');
			}
			xfer_section.push(`xfer b${dec_needle + 1} f${dec_needle + 2}`); //* f${dec_needle + 2} //*
		...
		if (!emptyRow.includes(`f${dec_needle + 1}`) && (!emptyRow.includes(`b${dec_needle + 2}`) || !emptyRow.includes(`f${dec_needle + 2}`))) { //new //check //*
			if (emptyRow.includes(`f${dec_needle + 2}`) && emptyRow.includes(`b${dec_needle + 3}`)) { //prevent triple stack
				...
				xfer_section.push(`xfer f${dec_needle + 1} b${dec_needle + 3}`); //* b${dec_needle + 2} //* //TODO: check to make sure (!emptyRow.includes(`b${dec_needle + 3}`) || !emptyRow.includes(`f${dec_needle + 3}`))
				if (emptyRow.includes(`b${dec_needle + 3}`)) {
					if (racking !== 0) {
						racking = 0;
						xfer_section.push('rack 0');
					}
					xfer_section.push(`xfer b${dec_needle + 3} f${dec_needle + 3}`); //* b${dec_needle + 2} //*
				}
			} else {
				racking = -1;
				xfer_section.push('rack -1');
				xfer_section.push('x-add-roller-advance 50');
				xfer_section.push(`xfer f${dec_needle + 1} b${dec_needle + 2}`); //* b${dec_needle + 2} //*
				if (emptyRow.includes(`b${dec_needle + 2}`)) {
					xfer_section.push(`xfer b${dec_needle + 1} f${dec_needle + 2}`); //new
				}
			}
		}
		...
		// 	xfer_section.push(`xfer f${dec_needle + 2} b${dec_needle + 3}`); //* b${dec_needle + 3} //*
		// }
		...
		if (!emptyRow.includes(`b${dec_needle - 1}`) && (!emptyRow.includes(`f${dec_needle - 2}`) || !emptyRow.includes(`b${dec_needle - 2}`))) { //new //check //?
			if (racking !== -1) {
				racking = -1;
				xfer_section.push('rack -1'); //new //check
			}
			xfer_section.push(`xfer b${dec_needle - 1} f${dec_needle - 2}`); //* f${dec_needle + 2} //*
			if (emptyRow.includes(`f${dec_needle - 2}`)) {
				racking = 0;
...
const BINDOFF = (xfer_needle, count, side, double_bed, xfer_section) => {
	...
	const posLoop = (op, bed) => {
		pos: for (let x = xfer_needle; x < xfer_needle + count; ++x) {
			...
			if (op === 'bind') {
				...
				xfer_section.push('rack 0');
				if (x !== xfer_needle) {
					// if (x > xfer_needle + 3) xfer_section.push('x-add-roller-advance -50');
					if (x > xfer_needle + 3) {
		...
		neg: for (let x = xfer_needle + count - 1; x >= xfer_needle; --x) {
			...
			if (op === 'bind') {
				...
				if (x !== xfer_needle + count - 1) {
					// if (x < xfer_needle + count - 4) xfer_section.push('x-add-roller-advance -50'); //remove //?
					if (x < xfer_needle + count - 4) {
						if (bindoff_time && x < xfer_needle + count - 21) xfer_section.push('x-add-roller-advance -300'); //new //* //check /-11 & -200 instead?
						else xfer_section.push('x-add-roller-advance -50');
					}
					xfer_section.push(`drop b${x + 1}`);
				}
	...
	if (side === 'left') {
		...
		if (!shortrow_time) xfer_section.push(`tuck - b${xfer_needle - 1} ${bindoff_carrier}`); //new (now not for just bindoff time) //TODO: check if !shortrow_time works
		...
		if (!shortrow_time) xfer_section.push(`tuck + b${xfer_needle + count} ${bindoff_carrier}`); //new //*
...
const inc2DoubleBed = (Xside_needle, side, inc_carrier, arr) => { //TODO: fix this for split
	if (side === 'left') {
		if (inc_method === 'xfer') {
			...
			arr.push(`miss - f${Xside_needle-2} ${inc_carrier}`); //miss carrier out of the way //new
		} else {
			twist = 0;
		}
		LtwistedF = true;
...
main: for (let r = xfer_row_interval; r < rows.length; r += xfer_row_interval) {
	...
	if (r !== 0) {
		...
		findKnitPass: for (let i = 0; i < rows[r].length; ++i) { //first pass
			lastKnitLn = rows[r][i].slice().reverse().find(ln => ln.includes('knit ')); //new //v
			if (lastKnitLn) {
				// insertL8rP = i;
				break findKnitPass;
			}
		}
		// let lastKnitLn = rows[r][0].slice().reverse().find(ln => ln.includes('knit ')); //new //v
		...
		for (let i = 0; i < rows[r - 1].length; ++i) {
			if (rows[r - 1][i].some((el) => el.includes(' - '))) {
				findKnit: for (let ln = rows[r - 1][i].length - 1; ln >= 0; --ln) { //new //v
			...
			for (let p = 0; p < rows[i].length; ++p) {
				if (back_passRneg.length > 0) {
					...
					for (let c = 0; c < back_passRneg.length; ++c) {
						...
						// ) { //new //?
							insert_arr.push(`;pass: back ;-;${back_passRneg[c][1]};${short_Xright_needle};${short_Xleft_needle}`); //check
							let mod = 3;
		...
		if (r <= last_shape_row && r >= shaping_arr[shaping_arr.length - 1].ROW && short_row_section) { 
			...
			bindoffC: for (let p = rows[first_short_row].length - 1; p >= 0; --p) {
				...
				if (rows[first_short_row][p].some((el) => el.includes(' - ')) && !bind_chosen) {
					...
					let srP = rows[first_short_row][p]; //new //v //* //* //*
					let srC;
			...
			if (bindoff_pass === undefined) {
				...
				let sr1 = rows[first_short_row][passIdx]; //new //v //* //* //*
				let sr1C;
			...
			if (!errors) {
				if (shortrow_colors.length < 3 && !carriers.includes(draw_thread)) { //TODO: make back pass an extra seed pass instead to comply with pattern
					if (shortrow_colors.length === 1) {
						...
						let left_carrier = findCarrierOnSide('left', first_short_row, bindoff_pass); //beep //new //*
						// let left_carrier = final_carrier_pos.find((el) => el.SIDE === 'left'); //go back! //?
						if (left_carrier !== undefined) {
							// let carrier_idx = new_carriers.indexOf(left_carrier.CARRIER); //go back! //?
							let carrier_idx = new_carriers.indexOf(left_carrier); //beep //new //*
							short_row_carriers.splice(short_row_carriers.indexOf(draw_thread), 1);
							short_row_carriers.splice(carrier_idx, 0, draw_thread);
						}
					}
			...
			if (!carriers_defined) {
				...
				for (let i = 0; i < tracked_carriers.length; ++i) { //new //* //v
				...
				// for (let i = 0; i < final_carrier_pos.length; ++i) { //go back! //? //v
			...
			// for (let c = 0; c < final_carrier_pos.length; ++c) { //TODO: make this only for color carriers //go back! //? //v
			...
			shaped_rows.push(cookie);
			// bindoff_carrier = cookie[cookie.length - 1].charAt(cookie[cookie.length - 1].length - 1); //TODO: change this !
			bindoff_carrier = cookie.slice().reverse().find(ln => ln.includes('knit ')).slice(-1); //new //v
			xfer_section.push(`x-speed-number ${xfer_speed_number}`);
			xfer_section.push('x-roller-advance 50');
			...
			find_dir: for (let y = first_short_row; y < rows.length; ++y) {
				...
				parse_pass: for (let i = 0; i < rows[y].length; ++i) {
					...
					if (rows[y][i].some((el) => el.includes(' - '))) {
						...
						// 		back_passRneg.push([y, last_op.charAt(last_op.length - 1), back_passRneg.length]); //beep
						// 	} else {
			...
			for (let rC = 0; rC < redefine_carriers.length; ++rC) { // rows[r][i].slice().reverse().find(ln => ln.includes('knit ')); //new //v
			...
			if (rows[first_short_row][rows[first_short_row].length - 1] !== rows[first_short_row][bindoff_pass]) { //bindoff pass isn't the last one
				...
				for (let p = bindoff_pass + 1; p < rows[first_short_row].length; ++p) { //here! //*
					// let srP = rows[first_short_row - 1][p]; //new //v //* //* //*
					let srP = rows[first_short_row][p]; //new //v //* //* //*
					let srC;
					findC: for (let ln = 0; ln < srP.length; ++ln) {
              	...
              	//     rows[first_short_row - 1][p][1].charAt(rows[first_short_row - 1][p][1].length - 1) === short_row_carriers[new_carriers.indexOf(back_passLpos[c][1])])
					...
					if (back_passRneg.length > 0) {
						...
						for (let c = 0; c < back_passRneg.length; ++c) {
							...
							if (first_short_row === back_passRneg[c][0] && (srC === back_passRneg[c][1] || srC === new_carriers[short_row_carriers.indexOf(back_passRneg[c][1])])) { //new //*
			...
			xfer_row_interval = first_short_row - r + 1;  // +1 (new)
		}
	}
...
xfer_section.push(`x-speed-number ${xfer_speed_number}`, 'x-roller-advance 100'); //new //*
