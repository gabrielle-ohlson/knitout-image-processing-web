...
const EMPTY = ({ FRONT, BACK }) => ({ //TODO: maybe remove since doesn't see to be used //?
	FRONT,
	BACK
});
...
// --------------------------------------------------------------------
let piece_code1, piece_shortrow_codeL, piece_shortrow_codeR; //removed Template, 
...
function generateKnitout(in_file, shape_code, shape_code_reverse, shortrow_code, short_row_section, first_short_row, last_short_row, section_count, shape_error, shape_err_row, inc_method, xfer_speed_number, console, chalk) {
	// ------------------------------------------------------------
	// --- GET SHAPING INFO FOR SHORT ROWING (if applicable) --- //
	// ------------------------------------------------------------
	let shortrow_sides = [];
	...
	if (shape_code_reverse !== null) { //TODO: remember what it would mean if it were to be null
	...
	// --------------------------------------------
	// --- SPLIT SOURCE FILE ARRAY INTO ROWS --- //
	// --------------------------------------------
	...
	// ------------------------------------------
	// --- DEAL WITH INFO FROM SOURCE FILE --- //
	// ------------------------------------------
	...
	if (short_row_carriers.length < 3 && short_row_section && !sinkers) {
		console.log(
			chalk.red.bold(`\nERR:`),
			chalk.red(` the section of the panel that will be altered to include short-rowing contains ${6 - short_row_carriers.length} colors, but the maximum color-count for that section is 3 (to allow for separate carriers to work either side while short-rowing).`)
		);
		errors = true;
	}
	...
	main: for (let r = xfer_row_interval; r < rows.length; r += xfer_row_interval) {
					...
					if (shortrow_time && back_passLpos.length > 0) {
						...
						for (let c = 0; c < back_passLpos.length; ++c) {
							if (i === back_passLpos[c][0] && (pass_carrier === back_passLpos[c][1] || pass_carrier === short_row_carriers[new_carriers.indexOf(back_passLpos[c][1])])) {
								...
								console.log(chalk.gray('adding back pass.')); //debug
		...
		if (shape_code_reverse !== null) {
			...
			if (r <= last_shape_row && r >= shaping_arr[shaping_arr.length - 1].ROW && short_row_section) { //TODO: if rows[r].length < 2 && it isn't a negative pass, no rows[first_short_row] instead 
				...
				if (r === shaping_arr[shaping_arr.length - 1].ROW) {
					...
					if (!negPass) {
						console.log(chalk.gray('no negative pass, starting first short row one row late.')); //debug
						last_shape_row += 1;
						first_short_row += 1;
						continue main;
					}
				...
				// add xtra_neg_carriers and xtra_pos_carriers to carrier_track if not in there yet
				for (let c = 0; c < xtra_neg_carriers.length; ++c) {
					if (!(xtra_neg_carriers[c] in carrier_track)) carrier_track[xtra_neg_carriers[c]] = {0: { 'side': 'left', 'idx': -1}}; //beep
				}

				for (let c = 0; c < xtra_pos_carriers.length; ++c) {
					if (!(xtra_pos_carriers[c] in carrier_track)) carrier_track[xtra_pos_carriers[c]] = {0: { 'side': 'right', 'idx': -1}};
				}
		...
		if (!short_row_section && r + xfer_row_interval > shaping_arr[shaping_arr.length - 1].ROW) {
			console.log(chalk.gray(`Heads up that no more shaping happens after row ${r} (generating warning because xfer_row_interval ${xfer_row_interval} is too large and the shaping part of the piece only goes up to row ${shaping_arr[shaping_arr.length - 1].ROW}).\n`)); //debug
			warning = true;
		}
	...
	if (rib_top !== undefined) {
		if (!short_row_section) {
			...
			console.log(
				...
				chalk.bold(` This program does not currently support ribbing at the top of pieces with short rowing, so the top rib section will be removed.`)
	...
	if (shaped_rows.some((line) => line.includes(undefined) || line.includes(NaN) || line.includes(null))) {
		console.log(chalk.red.bold(` ERR:`), chalk.red(` file includes invalid value such as 'undefined'.`));
		errors = true;
	}
	...
	if (!errors && !shape_error) {
		console.log(
			chalk.green(`\nno errors found :-)\n`),
			chalk.black.bgYellow(`! WARNING:`),
			chalk.bold(` IT IS RECOMMENDED THAT YOU VIEW THE NEW FILE ON THE KNITOUT LIVE VISUALIZER `),
			chalk.italic(`(https://textiles-lab.github.io/knitout-live-visualizer/)`),
			chalk.bold(` BEFORE USE TO ENSURE THAT IT WILL PRODUCE A KNIT TO YOUR LIKING.\n***contact: `),
			chalk.italic(`info@edgygrandma.com`),
			chalk.bold(` if you have any questions about this program.`)
		);
	}
	...
	// ----------------------------------
	if (!errors && !shape_error) return final_file; //new //*
	else {
		if (shape_error) {
			console.log(
				chalk.red(
		...
		return; //TODO: have throw error instead
	}
}
